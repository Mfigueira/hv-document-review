# HomeVision ù Review Page Challenge: Plan
...
primary action (Re-upload / Submit) on the business rule ù deployed as a static
...
| 3 | [`03-design-options.md`](./03-design-options.md) | The chosen **Split View** layout (desktop + mobile wireframes), the Submitted Page, and the dev panel ù plus the two alternatives considered. |
...
## Key decisions (LOCKED ù updated after first review)
...
- **Scope:** The **Review Page** + a **minimal Submitted Page**. The Upload /
  Processing pages stay out of scope ù but we scaffold placeholder routes for
  them (the "Re-upload" CTA routes to the `/upload` stub).
- **Hosting:** Static build deployed to Vercel / Netlify / Cloudflare Pages.

## The resolution model (DECIDED ù the app's core interaction)
...
|----------|----------------|-------------|------------------|------------|
| **A ù Needs revision** (given `v2` mock) | critical + major + minor | **Re-upload** | all critical **and** major issues are checked/resolved | conceptually goes to Upload ? Processing ? new Review (out of scope; we stop here) |
| **B ù Clean** (minor-only mock) | minor only | **Submit** | always (no blocking issues) | navigates to the **Submitted Page** |
